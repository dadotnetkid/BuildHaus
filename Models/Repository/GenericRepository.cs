...
namespace Models.Repository
{
    public class GenericRepository<TEntity> where TEntity : class
    {
        ...
        public virtual TEntity Find(Expression<Func<TEntity, bool>> filter, string includeProperties = "", bool lazyLoad = true, bool proxyCreation = true, bool asNoTracking = false)
        {
            this.context.Configuration.LazyLoadingEnabled = lazyLoad;
            this.context.Configuration.ProxyCreationEnabled = proxyCreation;
            IQueryable<TEntity> query = dbSet;
            ...
            if (includeProperties != "")
                foreach (var includeProperty in includeProperties.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                    query = query.Include(includeProperty);
            if (asNoTracking)
                query.AsNoTracking().Where(filter).FirstOrDefault();
